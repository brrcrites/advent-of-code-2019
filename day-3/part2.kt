...
fun main(args: Array<String>) {
    ...
    // TODO(brrcrites): I'm assuming the reason we can't use 'indexOfFirst' here is the same reason we
    // can't use the 'intersect' function above, but we should fix this when we fix the above
    var distance: Double = Double.POSITIVE_INFINITY
    for (point in point_intersection) {
        println("Processing (${point.x},${point.y})")
        var new_distance: Double = 0.0
        for ((index,p) in points.get(0).withIndex()) {
            if (p.x == point.x && p.y == point.y) {
                new_distance += index
            }
        }
        for ((index,p) in points.get(1).withIndex()) {
            if (p.x == point.x && p.y == point.y) {
                new_distance += index
            }
        }
